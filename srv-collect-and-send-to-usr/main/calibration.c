...
// calibration SG table (user-provided values)
static const float sg_table[] = {1.000, 1.085, 1.061, 1.040, 1.030, 1.020, 1.010};

/* Number of calibration points inferred from table */
#define CAL_POINTS_COUNT (sizeof(sg_table) / sizeof(sg_table[0]))

static CalPoint points[CAL_POINTS_COUNT];
...
void tilt_queue_push(float tilt)
{
    ...
    if (tilt_queue.count < TILT_QUEUE_SIZE)
        tilt_queue.count++;
    else
        tilt_queue.tail = (tilt_queue.tail + 1) % TILT_QUEUE_SIZE;
}
...
{
    /* Build normal equations for least-squares fit of cubic polynomial
       coeffs: sg = A3*t^3 + A2*t^2 + A1*t + A0
       We compute ATA (4x4) = V^T * V and ATy (4) = V^T * Y then solve ATA * coeffs = ATy
    */
    float ATA[4][4] = {0};
    float ATy[4] = {0};

    for (int i = 0; i < (int)CAL_POINTS_COUNT; i++)
    {
    ...
    /* Solve 4x4 system */
    solve_linear_system(ATA, ATy, coeffs, 4);
    complete = true;
...
{
    if (cal_state == CAL_STATE_READY && cal_point < (int)CAL_POINTS_COUNT)
    {
...
void calibrate_module_stop(uint16_t conn_handle)
{
    if (cal_state == CAL_STATE_MEASURING)
    {
        ...
        cal_point++;
        if (cal_point >= (int)CAL_POINTS_COUNT)
        {
