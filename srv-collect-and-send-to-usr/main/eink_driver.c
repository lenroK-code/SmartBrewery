...
static const uint8_t font6x8[] = {
    ...
    0x18, 0x3C, 0x3C, 0x18, 0x18, 0x00, // deg. (degree)
              ...
              int width, int height)
{
    ...
    // Configure GPIOs
    if (g_dc >= 0)
        gpio_config(&(gpio_config_t){.pin_bit_mask = 1ULL << g_dc, .mode = GPIO_MODE_OUTPUT});
    if (g_rst >= 0)
        gpio_config(&(gpio_config_t){.pin_bit_mask = 1ULL << g_rst, .mode = GPIO_MODE_OUTPUT});
    if (g_busy >= 0)
        gpio_config(&(gpio_config_t){.pin_bit_mask = 1ULL << g_busy, .mode = GPIO_MODE_INPUT});
...
#ifdef HSPI_HOST
        if (th_count < 6)
            tried_hosts[th_count++] = HSPI_HOST;
#endif
#ifdef VSPI_HOST
        if (th_count < 6)
            tried_hosts[th_count++] = VSPI_HOST;
#endif
#ifdef SPI2_HOST
        if (th_count < 6)
            tried_hosts[th_count++] = SPI2_HOST;
#endif
#ifdef SPI3_HOST
        if (th_count < 6)
            tried_hosts[th_count++] = SPI3_HOST;
#endif
